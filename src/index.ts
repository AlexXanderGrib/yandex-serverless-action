...
async function run() {
  ...
  try {
    const inputs: ActionInputs = {
      ...
      onlyFiles: core.getInput("only_files", { required: false })
    };

    core.info("Function inputs set");

    const fileContents = await (inputs.source.endsWith(".zip")
      ? fs.readFile(inputs.source)
      : zipDirectory(inputs));

    core.info(`Buffer size: ${fileContents.byteLength}b`);
    ...
    if (inputs.bucket) {
      ...
      const storageObject = StorageObject.fromBuffer(
        inputs.bucket,
        bucketObjectName,
        fileContents
      );
...
function handleOperationError(operation: Operation) {
  if (operation.error) {
    ...
    if (details)
      throw Error(
        `${operation.error.code}: ${operation.error.message} (${details.join(
          ", "
        )})`
      );
...
async function getFunctionById(
  ...
  try {
    ...
    const foundFunction = await functionService.get({
      functionId: inputs.functionId
    });

    if (foundFunction) {
      core.info(
        `Function found: "${foundFunction.id} (${foundFunction.name})"`
      );
...
async function createFunctionVersion(
  ...
  try {
    ...
    } else request.content = fileContents;
...
function parseIgnoreGlobPatterns(ignoreString: string): string[] {
  ...
  patterns.forEach((pattern) => {
    ...
    if (pattern?.length > 0) result.push(pattern);
  });
